...
def _segm_srun(pretrained, args, criterion):
    ...
    if pretrained:
        ...
        checkpoint = torch.load(os.path.join(os.getcwd(), "pretrained_weights/stvit-small-224.pth"), map_location='cpu')
        backbone.sp_vis_encoder.load_state_dict(checkpoint['model'], strict=True)
    else:
    ...
    model = SRUN(backbone, pixel_decoder, args, num_classes=num_classes, criterion=criterion)
    return model

def srun(pretrained='', args=None, criterion=None):
    return _segm_srun(pretrained, args, criterion)
