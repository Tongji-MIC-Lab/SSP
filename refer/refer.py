...
class REFER:
    ...
    def __init__(self, data_root, ref_root=None, dataset='refcoco', splitBy='unc', mix=False):
        ...
        if ref_root is None: # if not specified
            ref_root = data_root
        self.DATA_DIR = osp.join(data_root, dataset)
        if dataset in ['refcoco', 'refcoco+', 'refcocog', 'refcocom']:
    ...
    def showRef(self, ref, seg_box='seg'):
        ...
        I = cv2.imread(osp.join(self.IMAGE_DIR, image['file_name']), cv2.IMREAD_COLOR)
    ...
    def getMask(self, ref):
        ...
        area = mask.area(rle)  # should be close to ann['area']
